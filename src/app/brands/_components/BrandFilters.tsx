...
type BrandFiltersProps = Readonly<{
  pageSize: number;
  sort: BrandSort;
  status?: BrandStatusFilter;
}>;

const FILTERS_TOAST_ID = 'brands-filters-loading';
...
export default function BrandFilters({
  ...
  return (
    <form ref={formRef} method='GET' action={NAVIGATION.BRANDS}>
      <Grid gap={8} columns={4} alignItems='end'>
        ...
        <Grid display='flex' gap={8} alignItems='center'>
          <Button href={NAVIGATION.BRANDS} variant='borderless'>
            Reset
          </Button>
        </Grid>
      </Grid>
