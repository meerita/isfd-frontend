...
// File: src/_actions/user/updateUserAccess.ts
// Purpose: Update user access permissions via PATCH using UUID and shared axios/auth helpers
...
export type UserAccessPayload = NonNullable<User['access']>;

type UpdateUserAccessPayload = Readonly<{
  uuid: string;
  ...
  if (!normalizedUuid) {
    return {
      ...
      error: {
        ...
        message: 'Missing user identifier to update access.',
